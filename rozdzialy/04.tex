...
$$\bigwedge\nolimits^2E^*=\{B:E\times E\to \R\;:\;\text{dwuliniowe, }B(e,f)=-B(f,e)\}$$
...
\begin{example}[m]
  \item Annulus (znany też jako pierścień kołowy) to obszar płaszczyzny zespolonej pomiędzy dwoma okręgami: $A=\{z\in\C:r\leq|z|\leq R\}$. Punkty na annulusie możemy parametryzować przy pomocy promienia oraz kąta $(\rho, \theta)$.

    Pokażemy, że 
    $$H^1(A)=\ker d^1/\im d^0\neq 0,$$
    czyli wykażemy, że nie istnieje $F\in C^\infty(A)$ takie, że 1-forma $\d\theta$ jest równa $\d F$.

    Rozważmy krzywą 
    $$\gamma:[0,1]\to A$$
    $$\gamma(t)=(r, 2\pi t)$$
    zaznaczoną na obrazku.
    \begin{center}
      \begin{tikzpicture}
        \draw (0,0) circle (1);
        \draw (0,0) circle (2);
      ...
      \end{tikzpicture}
    \end{center}
    Gdyby istniało $F\in C^\infty(A)$ takie że $\d\theta=\d F$ to wówczas 
    $$\int_\gamma \d\theta=\int_\gamma \d F=F(\gamma(1))-F(\gamma(0))=0,$$ 
    ale całka z $\d\theta$ nie jest zerowa:
    $$\int_\gamma \d\theta=\theta(\gamma(1))-\theta(\gamma(0))=2\pi-0=2\pi.$$
...
<<<<<<< HEAD
  $\omega = R(x,y)dxdy$, $R \in C_c^\infty(\R^2)$
=======
  $\omega=R(x,y)dxdy$, $R\in C_c^\infty(\R^2)$
>>>>>>> 82ad1c3 (04)
  ...
  $$P(x,y) = \int_{-\infty}^y(\overline{R}(x,t)dt$$
  $P\in C_c^\infty(\R^2)$, $P_y=\overline{R}$
...
\begin{proof}
  ...
  Pokażemy, że $I_\gamma=I_\theta$: weźmy $\alpha\in\Omega^1(\Sigma)$, $d\alpha=0$ BZO: $\alpha|_{D_i}=0$
  $\alpha|_{D_i}\in\Omega^1(D_i)$, $d\alpha=0\implies\exists g_i\in\Omega^0(D_i)$ i $dg_i=\alpha$ wybieram $\chi_i\in\Omega^0_c(D_i)$, $\chi_i\equiv 1$ na $\gamma(t_i)\in D_i'\subseteq D_i$

  $\alpha\to \alpha-d(\chi_ig_i)\equiv 0$ na $D_i$

  $[\alpha]=[\alpha-d(\chi_ig_i)\in H^1(\Sigma)$

  $\alpha|_{U_i}=df_i$, $f_i\in\Omega^0(U_i)$ bo $H^1(U_i)=0$

  $\int_\Sigma\theta_i\wedge\alpha=\int_\Sigma \theta_i\wedge df_i=\int_\Sigma-d(\theta_i\cdot f_i)+\int_\Sigma d\theta_if_i$
  pierwsze ważne przejście to reguła Leibniza

  ze Stokes mamy że pierwszy składnik jest 0

  $\int_\Sigma d\theta_i\cdot f_i=\int_\Sigma f_id\theta_i=\int_\Sigma f_i\omega_{i+1}-f_i\omega_i=\int_{D_{i+1}}f_i\omega_{i+1}-\int_{D_i}f_i\omega_i$

  $\alpha|_{d_i}=0\implies f_i|_{D_i}=const$

  $\int_{D_{i+1}}f_i\omega_{i+1}-\int_{d_i}f_i\omega_i=f_i(\gamma(t_{i+1}))-f_i(\gamma(t_i))=\int_{\gamma|_{[t_i,t_{i+1}]}}df_i=\int_{\gamma|_{[t_i, t_{i+1}]}}\alpha$
\end{proof}
